...
public class CustomBakedGlyph implements BakedGlyph {
    ...
    public CustomBakedGlyph(int codepoint, CustomFontManager.GlyphData glyphData) {
        ...
        this.glyphInfo = () -> CustomFontRenderer.getInstance().getCachedScale() * glyphData.advance;
    }
    ...
    //?}
        ...
        public CustomTextRenderable(int codepoint, CustomFontManager.GlyphData glyphData, float x, float y, int color, int shadowColor, Style style,
        ...
        public void render(Matrix4f matrix4f, VertexConsumer vertexConsumer, int light, boolean isGui) {
            ...
            if (cfg.customFontShadow) {
                submitQuad(vertexConsumer, matrix4f, x0 + cfg.customFontShadowOffsetX + shadowOffset, y0 + cfg.customFontShadowOffsetY + shadowOffset,
                        ...
                        skew, boldStrength, ensureOpaque(cfg.customFontShadowColor), light);
                ...
                submitQuad(vertexConsumer, matrix4f, x0 + shadowOffset, y0 + shadowOffset,
                        ...
                        sdfGlyph.u0, sdfGlyph.v0, sdfGlyph.u1, sdfGlyph.v1,
                        skew, boldStrength, shadowColor, light);
            }

            if (cfg.customFontOutline) {
                submitQuad(vertexConsumer, matrix4f, x0, y0, x1, y1,
                        sdfGlyph.u0, sdfGlyph.v0, sdfGlyph.u1, sdfGlyph.v1,
                        skew, -(cfg.customFontOutlineWidth + boldStrength), ensureOpaque(cfg.customFontOutlineColor), light);
            }

            submitQuad(vertexConsumer, matrix4f, x0, y0, x1, y1,
                    sdfGlyph.u0, sdfGlyph.v0, sdfGlyph.u1, sdfGlyph.v1,
                    skew, boldStrength, color, light);
        }

        private void submitQuad(VertexConsumer vc, Matrix4f m,
                                float lx0, float ly0, float lx1, float ly1,
                                float u0, float v0, float u1, float v1,
                                float skew, float effectZ, int c, int light) {
        ...
        private VisualBounds getVisualBounds() {
            ...
            if (cfg.customFontBold || (style != null && style.isBold())) {
                boldPad = cfg.customFontBoldStrength + (boldOffset / Math.max(renderer.getCachedSdfRenderScale(), 0.001f));
                effectPad += boldPad;
            }
            if (cfg.customFontOutline) {
