...
public class RotationManager {
    ...
    private RotationManager() {
        HudElementRegistry.addLast(
                Identifier.fromNamespaceAndPath("blackaddons", "rotation_manager"),
                this::onFrame
        );
    ...
    private void renderOverlay(GuiGraphicsExtractor g, DeltaTracker tracker) {
        ...
        if (active) {
            g.text(mc.font, String.format("Yaw:   %.1f -> %.1f", curYaw, targetYawUnwrapped), 0, y, COLOR_WHITE);
            y += lineH;
            g.text(mc.font, String.format("Pitch: %.1f -> %.1f", curPitch, targetPitch), 0, y, COLOR_WHITE);
            y += lineH;

            float progress = durationTicks > 0 ? Math.max(0, currentTicks / durationTicks) : 1.0f;

            if (!Double.isNaN(targetX)) {
                g.text(mc.font, String.format("Pos: %.0f %.0f %.0f", targetX, targetY, targetZ), 0, y, COLOR_GRAY);
                y += lineH;
            }
            ...
            String extra = String.format("Spd: %.1f Curve: %.0f%%%s", ConfigManager.data.rotationSpeed, ConfigManager.data.rotationVariance * 100, ConfigManager.data.rotationHumanizerEnabled
                    ...
                    : "");
            g.text(mc.font, extra, 0, y, COLOR_GRAY);
            y += lineH;

            g.fill(0, y, OVERLAY_BAR_WIDTH, y + 4, COLOR_BAR_BG);
            g.fill(0, y, (int) (OVERLAY_BAR_WIDTH * progress), y + 4, COLOR_BAR_FILL);
            g.text(mc.font, String.format(" %.0f%%", progress * 100), OVERLAY_BAR_WIDTH, y - 2, COLOR_GRAY);
        }
        g.pose().popMatrix();
    }
